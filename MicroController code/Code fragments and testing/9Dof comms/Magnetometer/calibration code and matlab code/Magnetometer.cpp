...
int main()
{
  ...
  reg_out[0] = 0x18; //no sample averaging, 75Hx sample rate, normal measure mode (no self tiest bias)
  ...
  int mout = sd_mount(MOSI, SCLK, MISO, SS);
  ...
  while(1)
  {
      ...
      //x = twos2signed(buffer[4], buffer[5]);
      //y = -twos2signed(buffer[0], buffer[1]);
      //z = twos2signed(buffer[2], buffer[3]);
      
      x = ((buffer[4]<<8) | buffer[5])+150;
      y =  (-  ((buffer[0]<<8) | buffer[1]))-132;
      z = ((buffer[2]<<8) | buffer[3])+135;
      /*
      ...
      */
      ...
      FILE* fp = fopen("mag_data.txt", "a");          // Open a file for writing

      fprintf(fp,"1, 1, 1, 1, 1, 1, %f, %f, %f\n",_x, _y, _z); //need to add 6 dummies for the way the matlab code was written
      
      fclose(fp);     


      //print("\nX: %d  Y: %d  Z: %d, Heading: %f, RSS: %f", x, y, z, angle , RSS);
