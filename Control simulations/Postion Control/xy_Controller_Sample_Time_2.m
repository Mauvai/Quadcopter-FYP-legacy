...
for i=1:length(wc)
C=pidtune(sys_x_d,'pid',wc(i));
T_pi=feedback(sys_x_d*C,1);
step(T_pi)
hold on
xlim([0 150])
end
legend(['BW of ',num2str(wc(1))],['BW of ',num2str(wc(2))],['BW of ',num2str(wc(3))],['BW of ',num2str(wc(4))],['BW of ',num2str(wc(5))],'Location','Best')
%legend(['BW of ',num2str(wc(1))],['BW of ',num2str(wc(2))],['BW of ',num2str(wc(3))],'Location','Best')
title('Step Response of position under PID control')
ylabel('Position [m]')
grid on
saveas(gcf,'pid_xy_response.png')

C=pidtune(sys_x_d,'pid',0.4);
Kp_x = get(C,'Kp');
Ki_x = get(C,'Ki');
Kd_x = get(C,'Kd');
...
% Convert to Digital State Space System
A_x_d = sys_d.a;
B_x_d = sys_d.b;
C_x_d = sys_d.c;
D_x_d = sys_d.d;
...
save('xy_Controller_PID_D_Data','Kp_y','Ki_y','Kd_y','Kp_x','Ki_x','Kd_x','A_x_d','B_x_d','C_x_d','G_x_d')
