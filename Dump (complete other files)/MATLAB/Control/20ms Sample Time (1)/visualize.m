...
function multiplot(data, values, ind)
    ...
    xmin = min(data.t);
    xmax = max(data.t);
    ymin = 1.1*min(min(transpose([data.theta;data.angvel])));
    ymax = 1.1*max(max(transpose([data.theta;data.angvel])));
    grid on
%     ymin = -10%1.1 * min(min(values)); %%%%%%%%%%%%%%
%     ymax = 10%1.1 * max(max(values));
