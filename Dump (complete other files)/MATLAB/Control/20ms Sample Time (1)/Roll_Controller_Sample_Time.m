...
R = 1;
[K_Roll_Reg] = dlqr(A_d,B_d,Q,R);
K_Roll_i = 0;
K_Roll_d =K_Roll_Reg(1:end);

sys_cl = ss(A_d-B_d*K_Roll_Reg,B_d,C_d,0);

% Observer Design without Integrator
dPoles = eig(sys_cl);
dPole = max(real(dPoles))^15;

P = [dPole, dPole+.01, dPole+.02];
G_Roll_d = place(A_d',C_d',P)';
% Simulink without Integrator
A_Roll_d=A_d;
B_Roll_d=B_d;
C_Roll_d=C_d;
model = 'Roll_Diff_SS_D_No_Integrator';
sim(model)


%% LQR Method with Integrator
%Q=diag([0.1 0.001 0.001 5])
%Q=diag([1.0 0.001 0.001 5])
%Q=diag([1.25 0.001 0.001 5])
Q=diag([1.25 0.001 0.001 10])
%Q=diag([1.5 0.001 0.001 5])
%Q=diag([2.0 0.001 0.001 5]) Works well in simulations
%Q=diag([3.0 0.001 0.001 5])
